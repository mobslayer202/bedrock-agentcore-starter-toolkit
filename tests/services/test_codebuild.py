...
class TestCodeBuildService:
    ...
    def mock_clients(self, mock_session):
        ...
        # Configure S3 mock
        clients["s3"].head_bucket.side_effect = ClientError(
        ...
        clients["codebuild"].start_build.return_value = {
    ...
    def test_ensure_source_bucket_us_east_1(self, mock_session, mock_clients):
        ...
        bucket_name = service.ensure_source_bucket("123456789012")
    ...
    @patch("os.unlink")
    def test_upload_source_success(self, mock_unlink, mock_tempfile, mock_zipfile, mock_walk, 
                                 codebuild_service, mock_clients):
        """Test successful source upload."""
        # Mock file system
        mock_walk.return_value = [
            (".", ["subdir"], ["file1.py", "file2.txt"]),
            ("./subdir", [], ["file3.py"])
        ]
    ...
    def test_create_codebuild_execution_role_existing(self, codebuild_service, mock_clients):
        ...
        mock_clients["iam"].create_role.side_effect = mock_clients["iam"].exceptions.EntityAlreadyExistsException()
    ...
    def test_start_build(self, codebuild_service, mock_clients):
        ...
        mock_clients["codebuild"].start_build.assert_called_once_with(
            ...
            sourceLocationOverride="bucket/source.zip"
        )
...
"""
    ...
    def test_source_upload_with_dockerignore(self, codebuild_service, mock_clients):
        ...
        with patch("os.walk") as mock_walk, \
            ...
            mock_walk.return_value = [
                (".", [], ["test.py", "test.pyc", ".git", "README.md"])
            ]
    ...
    def test_iam_role_permissions(self, codebuild_service, mock_clients):
        ...
        ecr_statement = next(
            ...
            if "ecr:BatchCheckLayerAvailability" in stmt["Action"]
        )
        ...
        s3_statement = next(
            stmt for stmt in policy_doc["Statement"] 
            if "s3:GetObject" in stmt["Action"]
        )
